...
const shapes = {
  ...
  clock:
    "M4 0h8v2h2v2h2v8h-2v2h-2v2H4v-2H2v-2H0V4h2V2h2V0zm0 3v1H3v8h1v1h8v-1h1V4h-1V3H4zm3 1h2v4h3v2H7V4z",
};
export function PixelIcon({ name, ...props }: Props & { name: keyof typeof shapes }) {
  return (
    <svg
      ...
      {...props}
    >
      <path fillRule="evenodd" d={shapes[name]} />
      {name === "flame" && <path fill="var(--ember)" d="M7 8h2v3h2v3H6v-3h1z" />}
    </svg>
  );
}
export const Flame = ({ className, ...p }: Props) => (
  <svg
    ...
    aria-hidden="true"
    focusable="false"
    className={["living-flame", className].filter(Boolean).join(" ")}
    {...p}
  >
